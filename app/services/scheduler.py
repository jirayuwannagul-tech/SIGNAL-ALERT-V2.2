...
class SignalScheduler:
    ...
    def _is_duplicate_signal(self, symbol: str, timeframe: str, direction: str) -> bool:
        ...
        if signal_key in self.last_signals:
            ...
            if time_diff.total_seconds() < (self.cooldown_minutes * 60):
                remaining = self.cooldown_minutes - (time_diff.total_seconds() / 60)
                logger.info(f"🚫 [COOLDOWN] {signal_key} remains {remaining:.1f} mins")
                return True
    ...
    def set_services(self, signal_detector, position_manager, line_notifier, sheets_logger):
        ...
        # --- แทรกการสร้าง TelegramNotifier ---
        self.telegram_notifier = TelegramNotifier(
            token=self.config.get("telegram_token"),
            chat_id=self.config.get("telegram_chat_id")
        )
        logger.info("Services injected with Telegram support")
    ...
    def _process_signal_refactored(self, signal: Dict, timeframe: str) -> bool:
        try:
            ...
            pos_created = signal.get("position_created", False)
            direction = "LONG" if signals.get("buy") else "SHORT" if signals.get("short") else None
            
            if not symbol or strength < 75 or not direction: return False
            if self._is_duplicate_signal(symbol, timeframe, direction): return False
            if not pos_created:
                self._record_signal(symbol, timeframe, direction)
                return False
            
            # --- ส่งทั้ง LINE และ Telegram ---
            ...
            self._record_signal(symbol, timeframe, direction)
            logger.info(f"🚀 Dual-Alert: {symbol} {timeframe} {direction}")
            return True
        except Exception as e:
    ...
    def _update_positions_refactored(self):
        # ส่วนนี้ดึง Logic เดิมจากไฟล์ 621 บรรทัดของพี่กลับมา
        try:
