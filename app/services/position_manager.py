...
class PositionManager:
    ...
    # ✅ NEW: Backward-compatible events init
    ...
    def _ensure_events(self, position: Dict) -> Dict:
        ...
        if "events" not in position or not isinstance(position.get("events"), dict):
        ...
        position["events"].setdefault("TP1", None)

        position["events"].setdefault("TP2", None)

        position["events"].setdefault("TP3", None)

        position["events"].setdefault("SL", None)
    ...
    def update_positions(self) -> Dict[str, Dict]:
        ...
        try:
            ...
            for position_id, position in active_positions.items():
                ...
                # Ensure events exists for old positions
        ...
        try:
            ...
            for tp_name, tp_price in tp_levels.items():
                ...
                if not position["tp_hit"][tp_name]:
                    ...
                    if hit:
                        ...
                        # ✅ NEW: record TP event (timestamp persisted)
            ...
            if not position["sl_hit"]:
                ...
                if sl_hit:
                    ...
                    # ✅ NEW: record SL event (timestamp persisted)
    ...
    def _load_positions(self) -> Dict:
        ...
        try:
            ...
            for _k, p in positions.items():

                self._ensure_events(p)
