...
DEPRECATED (SIGNAL-ALERT V2.2):

- ระบบใช้งาน PositionManager เป็น source of truth สำหรับ positions แล้ว

- ไฟล์นี้เก็บไว้เพื่อ backward-compat / อ้างอิงเก่าเท่านั้น

อย่าเพิ่ม dependency ใหม่ที่ใช้ PositionTracker ใน production
...
class Position:
    ...
    def update_pnl(self, current_price: float) -> float:
        ...
        pnl_percent = calculate_pnl_pct(self.direction, self.entry_price, current_price)
        ...
        try:
            ...
            for i, tp_price in enumerate(position.take_profits, 1):
                ...
                if tp_key not in position.hit_tps:
                    ...
                    if tp_hit:
                        ...
                        new_tp_hits.append(
                            {"key": tp_key, "price": tp_price, "level": i}
                        )
            ...
            if new_tp_hits:
                ...
                for tp_hit in new_tp_hits:
                    ...
                    logger.info(
                        f"🎯 {symbol} {tp_key} HIT at {current_price} (target: {tp_price})"
                    )
            ...
            return {
                ...
                "take_profits": {
                    "TP1": (
                        ...
                        if len(position.take_profits) > 0
                        else None
                    ),
                    "TP2": (
                        position.take_profits[1]
                        if len(position.take_profits) > 1
                        else None
                    ),
                    "TP3": (
                        position.take_profits[2]
