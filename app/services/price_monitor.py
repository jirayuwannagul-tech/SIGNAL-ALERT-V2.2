...
class PriceMonitor:
    ...
    def __init__(self, config: Dict, sheets_logger=None):
        ...
        logger.info(
            f"PriceMonitor v2.0 initialized as coordinator (interval: {self.update_interval}s)"
        )
    ...
    def start_monitoring(self):
        ...
        self.monitor_thread = Thread(
            target=self._monitoring_loop, daemon=True, name="PriceMonitorCoordinator"
        )
                    ...
                    for position_id, update_info in updates.items():
                        ...
                        if update_info.get("position_closed"):
                            ...
                            tp_hits = [
                                ...
                                if k.endswith("_hit") and k.startswith("TP")
                            ]
            ...
            for position_id, update_info in updates.items():
                ...
                for tp_level in ["TP1", "TP2", "TP3"]:
                    ...
                    if update_info.get(tp_key, {}).get("hit", False):
                        ...
                        try:
                            ...
                            self.sheets_logger.log_tp_hit(
                                position_data, update_info[tp_key]
                            )
                ...
                if update_info.get("sl_hit", {}).get("hit", False):
                    ...
                    try:
                        ...
                        self.sheets_logger.log_sl_hit(
                            position_data, update_info["sl_hit"]
                        )
