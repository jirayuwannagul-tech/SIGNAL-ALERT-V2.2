...
class DataUpdater:
    ...
    def __init__(self):
        ...
        self.session.headers.update({"User-Agent": "Python/Trading-Bot-Updater"})
        ...
        with self.cache_lock:
            ...
            try:
                ...
                if timeframe not in self.cache[symbol] or force_reload:
                    self.cache[symbol][timeframe] = self.load_cache_from_files(
                        symbol, timeframe
                    )
                ...
                if latest_candles:
                    ...
                    updated_candles = self.merge_candles(
                        existing_candles, latest_candles
                    )
                    ...
                    logger.debug(
                        f"Updated cache for {symbol} {timeframe}: {len(self.cache[symbol][timeframe])} candles"
                    )
                else:
            ...
            for symbol in self.cache:
                for timeframe in self.cache[symbol]:
                    try:
                        ...
                        for month_key, month_candles in monthly_data.items():
                            ...
                            if os.path.exists(file_path):
                                try:
                                    ...
                                    pass
                            # รวมกับข้อมูลใหม่
                            merged_data = self.merge_candles(
                                existing_data, month_candles
                            )
                            # บันทึกลงไฟล์
                            data = {
                                "symbol": symbol,
                        ...
                        logger.error(
                            f"Failed to save cache for {symbol} {timeframe}: {e}"
                        )
    ...
    def start_auto_update(
        ...
        self.running = True
        logger.info(
            f"Starting auto update for {len(symbols)} symbols, {len(timeframes)} timeframes"
        )
        ...
        save_thread = Thread(target=self._save_worker, daemon=True, name="SaveWorker")
        save_thread.start()
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            **self.stats,
            ...
            "active_threads": len(
                [t for t in self.update_threads.values() if t.is_alive()]
            ),
        }
