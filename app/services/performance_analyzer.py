...
class PerformanceAnalyzer:
    ...
    def _clean_trading_record(self, record: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            if clean_record["symbol"] and clean_record["direction"] and clean_record["entry_price"] > 0:
                return clean_record
            return None
        except (ValueError, TypeError) as e:
        ...
        try:
            ...
            for trade in self.trading_data:
                ...
                if symbol not in symbols:
                    symbols[symbol] = {"total_trades": 0, "closed_trades": 0, "wins": 0, "losses": 0, "win_rate": 0}
            ...
            for symbol in symbols:
                ...
                if closed > 0:
                    symbols[symbol]["win_rate"] = round((symbols[symbol]["wins"] / closed) * 100, 1)
            sorted_symbols = sorted(symbols.items(), key=lambda x: x[1]["total_trades"], reverse=True)[:top_n]
    ...
    def analyze_signal_quality(self) -> Dict:
        ...
        try:
            ...
            return {"total_signals": total_signals, "signal_types": signal_types, "timeframes": timeframes, "signals_per_day": round(total_signals / max(30, 1), 1)}
        except Exception as e:
    ...
    def get_summary_stats(self) -> Dict:
        ...
        """
        if not self.trading_data:
            if not self.load_trading_data(7):
                return {"error": "No data available"}
        try:
            basic = self.calculate_basic_metrics()
            return {"total_trades": basic.get("total_trades", 0), "win_rate": basic.get("win_rate", 0), "active_positions": basic.get("open_trades", 0), "last_updated": datetime.now().strftime("%Y-%m-%d %H:%M:%S")}
        except Exception as e:
        ...
        try:
            ...
            for signal in self.signal_data:
                ...
                if timeframe not in timeframe_performance:
                    timeframe_performance[timeframe] = {"total_signals": 0, "signal_strength": []}
            ...
            for tf in timeframe_performance:
                ...
                timeframe_performance[tf]["avg_signal_strength"] = round(statistics.mean(strengths), 1) if strengths else 0
